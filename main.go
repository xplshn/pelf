...
func main() {
	log.Println("Starting Nova daemon")
...
func executeBundle(blob, param, outputFile string) string {
	log.Printf("Executing blob: %s with parameter: %s", blob, param)
	cmd := exec.Command(blob, param)
	output, err := cmd.Output()
	if err != nil {
		log.Printf("Bundle %s with parameter %s returned error code 1", blob, param)
		return ""
	}
